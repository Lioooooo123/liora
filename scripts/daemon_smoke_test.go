...
func TestCodingEvalScriptCoversTaskQualityBaseline(t *testing.T) {
	...
	content := string(data)
	for _, want := range []string{"Fake", "LIORA_PATCH_MODE=1", "LIORA_PERMISSION=prompt", "natural", "run_async", "task.diff", "task.patch_applied", "/timeline", "permission.requested", "permission.approved", "permission.denied", "task.cancelled", "coding eval ok"} {
		if !strings.Contains(content, want) {
