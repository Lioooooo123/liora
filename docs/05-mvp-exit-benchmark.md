...
- daemon API 覆盖健康检查、capabilities、memory list/search/add、workspace workbench snapshot、workspace timeline search、创建任务、按 workspace 查询任务、查询事件、单任务 SSE、多任务 SSE、diff、apply、cancel、approval、session/history/timeline；Go daemon client 需要覆盖 memory、workbench snapshot、timeline search 和多 task event stream；capabilities 需要包含内建工具和可用 MCP 工具。
