...
这个 eval 会启动临时 fake LLM 和 Core Daemon，覆盖自然语言规划、DOCX 文档读取、失败后 replan、单文件和多文件 patch-first 写入、apply 落盘、大输出截断、事件历史、timeline、permission approve/deny、cancel 和子进程清理。
