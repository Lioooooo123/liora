...
这个 smoke 会启动临时 fake LLM、fake MCP server、Core Daemon 和 `-tui-daemon` 交互入口，覆盖 `/tools` 的 MCP 工具展示、streaming 输出、`/timeline` 和运行中 `/cancel`。
...
这个 eval 会启动临时 fake LLM、fake MCP server 和 Core Daemon，覆盖自然语言规划、DOCX 文档读取、MCP 外部工具审批与调用、失败后 replan、单文件和多文件 patch-first 写入、apply 落盘、大输出截断、事件历史、timeline、permission approve/deny、cancel 和子进程清理。
