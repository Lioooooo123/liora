...
这个 smoke 会启动临时 fake LLM、fake MCP server、Core Daemon 和 `-tui-daemon` 交互入口，覆盖 `/tools` 的 MCP 工具展示、streaming 输出、`/tail` 历史回看、`/timeline` 和运行中 `/cancel`；`/diff` 预览由 Go 测试覆盖。
