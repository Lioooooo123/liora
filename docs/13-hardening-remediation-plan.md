...
- [ ] **Step 1: Confirm the failure** — `go test ./internal/daemon/ -run TestServerScheduleTriggerRejectsDisabledUnknownAndMalformed` → FAIL `missing_id` (405, want 404).
- [ ] **Step 2: Fix parsing** — preserve leading empty segment, tolerate one trailing slash:
...
- [ ] **Step 3: Re-run** the test → PASS, then `go test ./internal/daemon/` → PASS.
- [ ] **Step 4: Commit** `fix(daemon): return 404 for empty schedule id path segment`.
...
- [ ] **Step 1:** Add `ci.yml` triggering on `pull_request` and `push` (all branches), one `macos-14` job:
...
- [ ] **Step 3: Commit** `ci: run full go + protocol test suite on push and PR`.
...
- [ ] **Step 1:** Replace the subset line with `go test -count=1 ./...` and add `go vet ./...`.
- [ ] **Step 2: Commit** `ci(release): run full test suite before publishing`.
...
- [ ] **Step 1: Failing tests** in `store_test.go`:
...
- [ ] **Step 2: Run** → FAIL (status flips).
- [ ] **Step 3: Implement** — add the guard to both writes:
...
- [ ] **Step 4: Run** the new tests + `go test ./internal/task/ ./internal/daemon/` → PASS. If any existing test relied on overwriting a terminal state, inspect — it is asserting the bug.
- [ ] **Step 5: Commit** `fix(task): guard terminal status writes against races`.
...
- [ ] **Step 1: Failing test** `TestLatestEventsReturnsNewest`: append 1200 events, then a
...
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** `LatestEvents` (newest-N, returned ascending so existing
...
- [ ] **Step 4: Run** new + `go test ./internal/task/ ./internal/daemon/` → PASS.
- [ ] **Step 5: Commit** `fix(task): scan newest events for approval/diff/expiry lookups`.
...
- [ ] **Step 1: Failing tests** — create workspace with `evil -> <outside dir>` symlink; assert
  `read evil/secret`, `write evil/x`, and a patch targeting `evil/x` all error with "outside
  workspace" (skip on `runtime.GOOS == "windows"`).
- [ ] **Step 2: Run** → FAIL (escape succeeds).
- [ ] **Step 3: Implement** — after the lexical check, resolve the parent directory's real path
...
- [ ] **Step 4: Run** new + `go test ./internal/tools/ ./internal/apply/` → PASS.
- [ ] **Step 5: Commit** `fix(tools,apply): reject symlink escapes from workspace`.
...
- [ ] **Step 1: Failing test** `TestCreateRejectsNonAbsoluteWorkspace` (relative path → error)
  and `TestCreateRejectsMissingWorkspace` (nonexistent dir → error).
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** — require `filepath.IsAbs(workspace)`; `filepath.Clean`; `os.Stat`
...
- [ ] **Step 4: Run** `go test ./internal/task/ ./internal/daemon/` → PASS.
- [ ] **Step 5: Commit** `fix(task): require absolute existing workspace directory`.
...
- [ ] **Step 1: Failing test** — start a fake MCP server that echoes its env; assert
  `LIORA_LLM_API_KEY`/`OPENAI_API_KEY`/`LIORA_DAEMON_TOKEN` are absent but `PATH` present and
  configured env applied.
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** — build env by filtering `os.Environ()`, excluding keys matching
...
- [ ] **Step 5: Commit** `fix(mcp): filter secrets from MCP subprocess environment`.
...
- [ ] **Step 1: Failing test** — write a trace, assert `os.Stat(...).Mode().Perm() == 0o600`.
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** — `MkdirAll(..., 0o700)`; replace `os.Create` with
  `os.OpenFile(path, O_WRONLY|O_CREATE|O_TRUNC, 0o600)`.
- [ ] **Step 4: Run** → PASS.
- [ ] **Step 5: Commit** `fix(trace): restrict trace file permissions to owner`.
...
- [ ] **Step 1: Failing/behavioral test** — assert the daemon-mode server is constructed with a
  non-zero `ReadHeaderTimeout` (extract server construction into a small testable helper
  returning `*http.Server`).
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** — build `&http.Server{Addr, Handler, ReadHeaderTimeout: 10s,
...
- [ ] **Step 4: Run** `go test ./apps/cli/` → PASS; manual: `liora -daemon` + Ctrl-C exits cleanly.
- [ ] **Step 5: Commit** `fix(daemon): add HTTP timeouts and graceful shutdown`.
...
- [ ] **Step 1:** README:584 — replace "尚未实现本地 token 鉴权" with the real state
  (`-daemon-token` + `requireCapability` exist; default is no token). Note the embedded-daemon
  default has no token.
- [ ] **Step 2:** `package.json` — add `"test": "pnpm --filter @liora/protocol test"` so root
  `pnpm test` stops failing with `ERR_PNPM_NO_SCRIPT`.
- [ ] **Step 3:** Add `internal/trust/trust_test.go` covering `NormalizeSource`,
  `IsTrustedSource`, `IsUntrustedSource`, `LevelForSource` (0% → covered).
- [ ] **Step 4: Run** `go test ./internal/trust/` + `pnpm test` → PASS.
- [ ] **Step 5: Commit** `docs,test: correct README auth note, wire root pnpm test, cover trust`.
